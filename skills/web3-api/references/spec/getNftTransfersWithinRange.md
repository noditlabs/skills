...
> Retrieve all NFT transfers that occurred within a specified block range or date range across all contracts.
...
| `toBlock` | string | No | End block. Cannot be used with `fromDate`/`toDate` |
...
> **Tip**: Shorter date/block ranges yield faster responses.
...
| `logIndex` | integer | Yes | Log index |
...
| `function` | object | No | Function call info (only if `withFunction` is true) |
