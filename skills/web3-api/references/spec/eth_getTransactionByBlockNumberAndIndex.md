...
> Returns transaction information by block number and transaction index position.

- **Category**: Node API (JSON-RPC)
- **Official Docs**: https://developer.nodit.io/reference/eth_getTransactionByBlockNumberAndIndex
...
| `params[0]` - Block Number or Tag | `string` | Yes | A hex-encoded block number (e.g., `"0x1076B5A"`) or a block tag (`"latest"`, `"earliest"`, `"pending"`, `"safe"`, `"finalized"`) |
