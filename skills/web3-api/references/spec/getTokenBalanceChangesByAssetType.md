...
> Retrieve balance change history for a specific asset type on Aptos.
...
|-------|------|----------|-------------|
| assetType | string | Conditional | Asset type (Coin struct or FA object address). Either `assetType` or `linkedAssetType` required (mutually exclusive). |
| linkedAssetType | string | Conditional | Common identifier for Coin/FA pair. Either `assetType` or `linkedAssetType` required (mutually exclusive). |
...
| eventType | string | Yes | Event type (Move struct path, e.g., `0x1::coin::WithdrawEvent`). |
